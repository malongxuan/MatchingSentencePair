		...
		def get_core_model(model_param, embedding_file, vocab_size):
			...
			class _SoftAlignment(object):#for second co, output len*hidden
				...
				def __call__(self, sentence, attention, ques_len,max_length):
					...
					return merge([attention, sentence,ques_len], mode=_normalize_attention,
								  output_shape=(max_length, self.nr_hidden)) 
			class _SoftAlignment_2(object):#for second co, output len*hidden
				def __init__(self, nr_hidden):
					self.nr_hidden = nr_hidden
				def __call__(self, sentence, attention, max_length):
					def _normalize_attention(attmat):
						att = attmat[0]
						mat = attmat[1]
						att = K.permute_dimensions(att,(0, 2, 1))
					...
					return merge([attention, sentence], mode=_normalize_attention,
								  output_shape=(max_length, self.nr_hidden)) 
			class _SoftAlignment_nodiag(object):
				def __init__(self, nr_hidden):
					self.nr_hidden = nr_hidden
				def __call__(self, sentence, attention, max_length):
					def _batch_dot(x):
						return x-T.diag(T.nlinalg.diag(x))
					def _normalize_attention(attmat):
						att = attmat[0]
						mat = attmat[1]
						e,_ = theano.scan(_batch_dot,sequences=[att])
			...
			class _batch_dot_intra(object):#for intra and new-self, not max/mean, but diagonal, output 1*hidden
				def __init__(self, nr_hidden):
					self.nr_hidden = nr_hidden
				def __call__(self, sentence, attention, input_length, output_length=1):
					def _batch_dot(x):
					...
					def _normalize_attention(attmat):
						...
						return K.max(sm_att.dimshuffle((0,1,2,'x')) * mat.dimshuffle((0,'x',1,2)), axis=1, keepdims=False)
						
					return merge([attention, sentence], mode=_normalize_attention,
								  output_shape=(input_length,self.nr_hidden))
			
			class _max_self(object):#for self n, output 1*hidden
				def __init__(self, nr_hidden):
					self.nr_hidden = nr_hidden
				def __call__(self, sentence, attention, input_length, output_length=1):
					def _normalize_attention(attmat):
					...
					return merge([attention, sentence], mode=_normalize_attention,
								  output_shape=(output_length, self.nr_hidden)) 
			class _mean_self(object):#for self n, output 1*hidden
				def __init__(self, nr_hidden):
					self.nr_hidden = nr_hidden
				def __call__(self, sentence, attention, input_length, output_length=1):
					def _normalize_attention(attmat):
					...
					return merge([attention, sentence], mode=_normalize_attention,
								  output_shape=(output_length, self.nr_hidden)) 
			class _one_self(object):#for self 1, output 1*hidden
				def __init__(self, nr_hidden):
					self.nr_hidden = nr_hidden
				def __call__(self, sentence, attention, input_length, output_length=1):
					def _normalize_attention(attmat):
						...
						e = K.permute_dimensions(att,(0, 2, 1))
						g = K.exp(e - K.max(e, axis=-1, keepdims=True))
			...
			th_a_max = Softmax_self(answer_emb,temp_t_a,model_param.dec_timesteps)
			th_q_max = Softmax_self(question_emb,temp_t_q,model_param.enc_timesteps)
			th_a_mean = Softmean_self(answer_emb,temp_t_a,model_param.dec_timesteps)
			th_q_mean = Softmean_self(question_emb,temp_t_q,model_param.enc_timesteps)
			
			#======self 1*hidden, first compress to len*1, then softmax, then multiply, it is not real self
			one_self = _one_self(model_param.hidden_dim)

			tanh_q_D_1 = Dense(1,activation="tanh")
			tanh_q_1 = TimeDistributed(tanh_q_D_1,name="tanh_q_1")
			tanh_a_D_1 = Dense(1,activation="tanh")
			tanh_a_1 = TimeDistributed(tanh_a_D_1,name="tanh_a_1")
			temp_t_a_1 = tanh_a_1(answer_emb)
			temp_t_q_1 = tanh_q_1(question_emb)
			print "temp_t_a_1._keras_shape",temp_t_a_1._keras_shape
			print "temp_t_q_1._keras_shape",temp_t_q_1._keras_shape
			th_a_1 = one_self(answer_emb,temp_t_a_1,model_param.dec_timesteps)
			th_q_1 = one_self(question_emb,temp_t_q_1,model_param.enc_timesteps)
			...
			answer_th_max = max_pool(th_a_full)
			answer_th_avg = avg_pool(th_a_full)
			question_th_max = max_pool(th_q_full)
			question_th_avg = avg_pool(th_q_full)

			##======batch-dot attention, for intra new-intra and new-self
			diagonal_S = _batch_dot_intra(hidden_dim)
			
			tanh_q_D_len = Dense(model_param.enc_timesteps,activation="tanh")
			tanh_q_len = TimeDistributed(tanh_q_D_len,name="tanh_q_len")
			tanh_a_D_len = Dense(model_param.dec_timesteps,activation="tanh")
			tanh_a_len = TimeDistributed(tanh_a_D_len,name="tanh_a_len")
			temp_t_q_len = tanh_q_len(question_emb)
			temp_t_a_len = tanh_a_len(answer_emb)
			print "temp_t_q_len._keras_shape",temp_t_q_len._keras_shape
			print "temp_t_a_len._keras_shape",temp_t_a_len._keras_shape
			question_new_self = diagonal_S(question_emb,temp_t_q_len,model_param.enc_timesteps)
			answer_new_self = diagonal_S(answer_emb,temp_t_a_len,model_param.dec_timesteps)
			print "question_new_self._keras_shape",question_new_self._keras_shape
			print "answer_new_self._keras_shape",answer_new_self._keras_shape
			...
			#for new-co-att and new-intra-att
			q_D_full = Dense(model_param.hidden_dim)
			q_full = TimeDistributed(q_D_full,name="q_full")
			a_D_full = Dense(model_param.hidden_dim)
			a_full = TimeDistributed(a_D_full,name="a_full")
			ans_full = a_full(answer_emb)
			ques_full = q_full(question_emb)
			#the new-co-att
			new_ques_att = Attend(ques_full,answer_emb)
			new_ans_att = Attend(question_emb,ans_full,reverse = True)
			#the new-intra-att
			new_ques_intra = Attend_q(ques_full,question_emb)
			new_ans_intra = Attend_a(ans_full,answer_emb)
			print("new_ques_intra._keras_shape",new_ques_intra._keras_shape)
			print("new_ans_intra._keras_shape",new_ans_intra._keras_shape)
			...
			new_question_Max = Max_S(question_emb,new_ques_att,model_param.enc_timesteps)
			new_answer_Max = Max_S(answer_emb,new_ans_att,model_param.dec_timesteps)
			new_question_Mean = Mean_S(question_emb,new_ques_att,model_param.enc_timesteps)
			new_answer_Mean = Mean_S(answer_emb,new_ans_att,model_param.dec_timesteps)
			...
			#the new-co-att
			new_answer_Soft = Align_mask(ques_full,new_ques_att,ques_filter_repeat_len,model_param.dec_timesteps)
			new_question_Soft = Align_mask(ans_full,new_ans_att,ans_filter_repeat_len,model_param.enc_timesteps)
			print answer_Soft._keras_shape#(92,300)
			max_pool = Lambda(lambda x:K.max(x, axis=1,keepdims=True), output_shape=lambda x:(x[0], 1, x[2]))
			avg_pool = Lambda(lambda x:K.mean(x, axis=1,keepdims=True), output_shape=lambda x:(x[0], 1, x[2]))
			...
			new_answer_Soft_max = max_pool(new_answer_Soft)
			new_answer_Soft_avg = avg_pool(new_answer_Soft)
			new_question_Soft_max = max_pool(new_question_Soft)
			new_question_Soft_avg = avg_pool(new_question_Soft)
			...
			new_question_intra_Max = Max_S(question_emb,new_ques_intra,model_param.enc_timesteps)
			new_question_intra_Mean = Mean_S(question_emb,new_ques_intra,model_param.enc_timesteps)
			new_answer_intra_Max = Max_S(answer_emb,new_ans_intra,model_param.dec_timesteps)
			new_answer_intra_Mean = Mean_S(answer_emb,new_ans_intra,model_param.dec_timesteps)
			...
			question_intra_diag = diagonal_S(question_emb,ques_intra,model_param.enc_timesteps)
			answer_intra_diag = diagonal_S(answer_emb,ans_intra,model_param.dec_timesteps)
			new_question_intra_diag = diagonal_S(question_emb,new_ques_intra,model_param.enc_timesteps)
			new_answer_intra_diag = diagonal_S(answer_emb,new_ans_intra,model_param.dec_timesteps)
			...
			new_question_intra_Soft = Align_mask(ques_full,new_ques_intra,ques_repeat_ques,model_param.enc_timesteps)
			new_answer_intra_Soft = Align_mask(ans_full,new_ans_intra,ans_repeat_ans,model_param.dec_timesteps)
			print "answer_intra_Soft._keras_shape:",answer_intra_Soft._keras_shape#(90,300)
			print "new_answer_intra_Soft._keras_shape:",new_answer_intra_Soft._keras_shape#(90,300)
			...
			new_answer_intra_Soft_max = max_pool(new_answer_intra_Soft)
			new_answer_intra_Soft_avg = avg_pool(new_answer_intra_Soft)
			new_question_intra_Soft_max = max_pool(new_question_intra_Soft)
			new_question_intra_Soft_avg = avg_pool(new_question_intra_Soft)
			
			#intra no diagnal alignment, it is len*hidden
			Align_nodiag = _SoftAlignment_nodiag(hidden_dim)
			question_intra_nodiag = Align_nodiag(question_emb,ques_intra,model_param.enc_timesteps)
			answer_intra_nodiag = Align_nodiag(answer_emb,ans_intra,model_param.dec_timesteps)
			new_question_intra_nodiag = Align_nodiag(ques_full,new_ques_intra,model_param.enc_timesteps)
			new_answer_intra_nodiag = Align_nodiag(ans_full,new_ans_intra,model_param.dec_timesteps)
			
			question_emb = merge([question_emb,question_new_self,question_intra_diag,new_question_intra_diag],mode='concat',concat_axis=-1)
			answer_emb = merge([answer_emb,answer_new_self,answer_intra_diag,new_answer_intra_diag],mode='concat',concat_axis=-1)
			print "answer_emb._keras_shape:",answer_emb._keras_shape#(98,300)
			
			##=====encoder
			SigmoidDense = Dense(hidden_dim*4,activation="sigmoid")
			TanhDense = Dense(hidden_dim*4,activation="tanh")
			...
			print "answer_proj._keras_shape:",answer_proj._keras_shape

			#=====second co att
			Attend2 = _Attention(model_param.enc_timesteps, model_param.dec_timesteps, hidden_dim*4)
			Align = _SoftAlignment(hidden_dim*4)
			...
			answer_align = Align(question_proj,ques_atten_metrics,ques_filter_repeat_len,model_param.dec_timesteps)
			question_align = Align(answer_proj,ans_atten_metrics,ans_filter_repeat_len,model_param.enc_timesteps)
			print "answer_align._keras_shape:",answer_align._keras_shape
			...
			avg_pool = Lambda(lambda x: K.mean(x, axis=1, keepdims=False), output_shape=lambda x: (x[0], x[2]))
			press_max_sf_a = max_pool(answer_new_self)
			press_max_sf_q = max_pool(question_new_self)
			press_avg_sf_a = avg_pool(answer_new_self)
			press_avg_sf_q = avg_pool(question_new_self)
			press_max_it_a = max_pool(answer_intra_diag)
			press_max_it_q = max_pool(question_intra_diag)
			press_avg_it_a = avg_pool(answer_intra_diag)
			press_avg_it_q = avg_pool(question_intra_diag)
			press_max_nit_a = max_pool(new_answer_intra_diag)
			press_max_nit_q = max_pool(new_question_intra_diag)
			press_avg_nit_a = avg_pool(new_answer_intra_diag)
			press_avg_nit_q = avg_pool(new_question_intra_diag)
			max_qa = merge([press_max_sf_a,press_max_sf_q,press_max_it_a,press_max_it_q,press_max_nit_a,press_max_nit_q],mode='concat')
			avg_qa = merge([press_avg_sf_a,press_avg_sf_q,press_avg_it_a,press_avg_it_q,press_avg_nit_a,press_avg_nit_q],mode='concat')
			...
			feature_mean_max = merge([max_qa,avg_qa],mode='concat')
			qaDense = Dense(hidden_dim/10,activation="tanh")
			feature_self = qaDense(feature_mean_max)

			feature_total = merge([feature_all,feature_self],mode='concat')
			Score_total = Dense(hidden_dim/10)
			feature_final = Score_total(feature_total)

			Score_final = Dense(1)
			score = Score_final(feature_final)
