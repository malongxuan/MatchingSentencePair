...
def main(args):
	...
	if args.task == "wikiqa":  #ModelParam is just a tuple format, random_size means the neg_answ pool size
		model_param = ModelParam(hidden_dim=args.hidden_dim, enc_timesteps=24, dec_timesteps=60, batch_size=args.batch_size, random_size=15, lr=args.lr, k_value_ques=args.k_value_ques,k_value_ans=args.k_value_ans)
	elif args.task == "trecqa":
		model_param = ModelParam(hidden_dim=args.hidden_dim, enc_timesteps=24, dec_timesteps=60, batch_size=args.batch_size, random_size=15, lr=args.lr, k_value_ques=args.k_value_ques,k_value_ans=args.k_value_ans)
	elif args.task == "insqa":
		model_param = ModelParam(hidden_dim=args.hidden_dim, enc_timesteps=24, dec_timesteps=60, batch_size=args.batch_size, random_size=20, lr=args.lr, k_value_ques=args.k_value_ques,k_value_ans=args.k_value_ans)
